...
CREATE TABLE TODOUSER.project(id NUMBER GENERATED ALWAYS AS IDENTITY, name VARCHAR2(4000), PRIMARY KEY (id));
...
project_id NUMBER,
PRIMARY KEY (id),
FOREIGN KEY (project_id) REFERENCES TODOUSER.PROJECT(id) ON DELETE CASCADE,
FOREIGN KEY (manager_id) REFERENCES TODOUSER.EMPLOYEE(id) ON DELETE SET NULL
);

CREATE TABLE TODOUSER.todoitem (
id NUMBER GENERATED ALWAYS AS IDENTITY,
description VARCHAR2(4000), 
creation_ts TIMESTAMP WITH TIMEZONE DEFAULT CURRENT_TIMESTAMP,
status VARCHAR2(20) CHECK(status IN ('COMPLETED', 'PENDING', 'CANCELLED', 'REVIEWING')) DEFAULT 'PENDING',
assigned_dev_id NUMBER,
manager_id NUMBER,
completion_ts TIMESTAMP WITH TIMEZONE,
deadline DATE, 
project_id NUMBER, 
PRIMARY KEY (id), 
FOREIGN KEY (project_id) REFERENCES TODOUSER.PROJECT(id) ON DELETE CASCADE,
FOREIGN KEY (assigned_dev_id) REFERENCES TODOUSER.EMPLOYEE(id) ON DELETE SET NULL,
FOREIGN KEY (manager_id) REFERENCES TODOUSER.EMPLOYEE(id) ON DELETE SET NULL
);
...
id NUMBER,
todoitem_id NUMBER,
CONSTRAINT subtodoitem_pk PRIMARY KEY (id, todoitem_id),
FOREIGN KEY (todoitem_id) REFERENCES TODOUSER.todoitem(id) ON DELETE CASCADE
);


CREATE SEQUENCE TODOUSER.subtodoitem_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER TODOUSER.subtodoitem_trigger
BEFORE INSERT ON TODOUSER.subtodoitem
FOR EACH ROW
BEGIN
  IF :NEW.id IS NULL THEN
    SELECT subtodoitem_seq.NEXTVAL INTO :NEW.id FROM DUAL;
  END IF;
END;

INSERT INTO TODOUSER.project (name) VALUES ('Manual Insert Project');
commit;
!
