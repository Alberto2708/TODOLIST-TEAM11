...
export default function Stats() {
    ...
    try {
      ...
      const completedPercentages = await Promise.all(
        data.map(async (employee) => {
          const percentage = await fetchDeveloperCompletedPercentage(
            employee.id,
            sprintId
          );
          const workedHoursData = await fetchDeveloperWorkedHours(
            employee.id,
            sprintId
          );
  ...
  const fetchDeveloperCompletedPercentage = async (assignedDevId, sprintId) => {
    try {
      const response = await fetch(
        `/assignedDev/${assignedDevId}/sprint/${sprintId}/completedTasks/kpi`
      );
      const data = await response.json();
  ...
  const fetchDeveloperWorkedHours = async (assignedDevId, sprintId) => {
    try {
      const response = await fetch(
        `/assignedDev/${assignedDevId}/sprint/${sprintId}/workedHours/kpi`
      );
      const data = await response.json();
  ...
  const filteredWorkedHours =
    selectedDeveloper === "all"
      ? workedHours
      : chartData.find((entry) => entry.id === selectedDeveloper)
          ?.workedHours || 0;
