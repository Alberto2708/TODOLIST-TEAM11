...
export default function CompletedTasks() {
  ...
  const fetchSubTasks = async (taskId, assignedDevId) => {
    try {
      console.log(
        "Fetching subtasks for task ID:",
        taskId,
        "and employee ID:",
        assignedDevId
      );
      const response = await fetch(
        `/subToDoItems/toDoItem/${taskId}/employee/${assignedDevId}/completed`
      );
      if (!response.ok) {
  ...
  const fetchEmployees = async (managerId, sprintId) => {
    try {
      ...
      const data = await response.json();
      setEmployees(data);
      const taskPromises = data.map((employee) =>
        fetchTasks(employee.id, sprintId)
      );
  ...
  const fetchTasks = async (assignedDevId, sprintId) => {
    try {
      console.log(
        `Fetching tasks for assignedDevId: ${assignedDevId}, sprintId: ${sprintId}`
      );
      const response = await fetch(
        `/assignedDev/${assignedDevId}/sprint/${sprintId}/father/completed`
      );
      if (!response.ok) {
        console.error(
      ...
      console.log(
        `Parsed tasks for assignedDevId ${assignedDevId}:`,
        parsedTasks
      );
      ...
      {isScreenLoading ? (
        ...
        <>
          ...
          {employees
            .filter(
              (employee) =>
                selectedDeveloper === "all" || employee.id === selectedDeveloper
            )
            .map(
              (employee) =>
                tasks[employee.id] &&
                tasks[employee.id].length > 0 && (
